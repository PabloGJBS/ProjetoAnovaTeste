...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (document.trim().length < 5) {
      setError("Documento precisa ter pelo menos 5 caracteres");
      return;
    }
    ...
    setLoading(false);
    if (!result.ok) {
      setError(result.error || "Falha ao criar usuário");
      return;
    }

    setSuccess("Conta criada. Você pode entrar agora.");
    ...
    <div className="relative min-h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
      ...
      <div className="relative z-10 flex min-h-screen items-center justify-center px-6 py-12">
        <form
          ...
          className="w-full max-w-md rounded-3xl border border-white/10 bg-[var(--surface)]/95 p-8 shadow-[0_30px_80px_rgba(0,0,0,0.5)] backdrop-blur-xl"
        >
          <div className="space-y-2">
            <p className="text-[10px] uppercase tracking-[0.45em] text-[var(--muted)]">Cadastro</p>
            <h1 className="font-display text-3xl text-white">Criar conta</h1>
            <p className="text-sm text-[var(--muted)]">Conta padrão. Acesso admin é definido manualmente.</p>
          </div>
          ...
          {success && (
            <div className="mt-4 flex items-center gap-2 rounded-xl border border-[var(--success)]/30 bg-[var(--success)]/10 px-4 py-3 text-sm text-[var(--success)]">
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
